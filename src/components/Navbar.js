import React, { useState } from 'react';
import backgroundImage from '../assets/images/lib4.jpg'



const Navbar = ({ onSignUpClick,isLoggedIn }) => {
  // const [isOpen, setIsOpen] = useState(false);
  // const user_pic = "data:image/jpeg;base64,"
  const [isOpen, setIsOpen] = useState(false);


  return (
    <nav className=" fixed top-0 w-full  flex flex-row items-center justify-between flex-wrap p-6 bg-slate-100 border-b-2 shadow-lg border-blue-200 z-10">

      
        <div href="/" className="relative md:basis-1/2 flex items-center flex-shrink-0 text-black mr-6 lg:mr-72">
           <div className='text-black text-3xl font-mono md:pl-24 font-semibold'>HIGH PLAY</div>
        </div>

        <div className="md:basis-1/2 block lg:hidden">
            <button
              onClick={() => setIsOpen(!isOpen)}
              className="flex items-center px-3 py-2 rounded text-black-500 hover:text-black-400"
            >
              <svg
                className={`fill-current h-3 w-3 ${isOpen ? "hidden" : "block"}`}
                viewBox="0 0 20 20"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v-2z" />
              </svg>
              <svg
                className={`fill-current h-3 w-3 ${isOpen ? "block" : "hidden"}`}
                viewBox="0 0 20 20"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M10 8.586L2.929 1.515 1.515 2.929 8.586 10l-7.071 7.071 1.414 1.414L10 11.414l7.071 7.071 1.414-1.414L11.414 10l7.071-7.071-1.414-1.414L10 8.586z" />
              </svg>
            </button>
        </div>

        <div className={` w-full block flex-grow lg:flex lg:items-center lg:w-auto lg:ml-24 ${isOpen ? "block" : "hidden"}`}>
          <div className="text-sm lg:flex-grow">
            <a href="/" className="block mt-4 lg:inline-block lg:mt-0 text-white-200 text-lg  mr-10 hover:text-gray-400 ">Home</a>
            <a href="/ourWorks" className="block mt-4 lg:inline-block lg:mt-0 text-white-200 text-lg mr-10 hover:text-gray-400">Our Works</a>
            <a href="/about" className="block mt-4 lg:inline-block lg:mt-0 text-white-200 text-lg mr-10 hover:text-gray-400">About</a>
            {/* <a href="/contact" className="block mt-4 lg:inline-block lg:mt-0 text-white-200 text-lg mr-10 hover:text-gray-400">Contact</a>    */}
          </div>
        </div>
    
    
  </nav>
  );
};

export default Navbar;